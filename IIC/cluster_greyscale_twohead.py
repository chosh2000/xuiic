...
parser.add_argument("--model_ind", type=int, default=685)
parser.add_argument("--arch", type=str, default="ClusterNet6cTwoHead")
parser.add_argument("--opt", type=str, default="Adam")
parser.add_argument("--mode", type=str, default="IID")

parser.add_argument("--dataset", type=str, default="MNIST")
parser.add_argument("--dataset_root", type=str, default="/home/sanghyun/Documents/data")
...
parser.add_argument("--out_root", type=str, default="outroot")
...
parser.add_argument("--crop_orig", dest="crop_orig", default=True, action="store_true")
parser.add_argument("--crop_other", dest="crop_other", default=True, action="store_true")
parser.add_argument("--tf1_crop", type=str, default="centre_half")  # type name
parser.add_argument("--tf2_crop", type=str, default="random")
parser.add_argument("--tf1_crop_sz", type=int, default=20)
parser.add_argument("--tf2_crop_szs", type=int, nargs="+",
                    default=[16, 20, 24])  # allow diff crop for imgs_tf
...
parser.add_argument("--no_flip", dest="no_flip", default=True, action="store_true")
...
def train(render_count=-1):
  ...
  net = ClusterNet6cTwoHead(config)
  if config.restart:
      ...
      for head_i_epoch in range(head_epochs[head]):
        ...
        for tup in zip(*iterators):
          ...
          all_imgs = torch.zeros((config.batch_sz, config.in_channels,
                                  config.input_sz,
                                  config.input_sz)).to(config.device)
          ...
          for d_i in range(config.num_dataloaders):
            ...
            all_imgs[actual_batch_start:actual_batch_end, :, :, :] = \
              imgs_curr.to(config.device)
            all_imgs_tf[actual_batch_start:actual_batch_end, :, :, :] = \
              imgs_tf_curr.to(config.device)
